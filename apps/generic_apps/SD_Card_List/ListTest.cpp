...
void testAdd(){ //Test assumes getValueByIndex works fine - works!
	...
	wiselib::List<Os, int, int, Os::size_t, Os::size_t, 100, 200, 512, false> list(debug_, sd);
...
void testInsertByIndex(){ //Test assumes add & getValueByIndex works fine - works!
	...
	wiselib::List<Os, unsigned int, int, int, int, 100, 200, 512, false> list(debug_, sd);
...
void testGetKeyIndexBothWays(){ //assumes add works as it should
	...
	void test(){
	debug_->debug("Official Test start here");
	wiselib::List<Os, int, Message, Os::size_t, Os::size_t, 100, 200, 512, false> list(debug_, sd);
