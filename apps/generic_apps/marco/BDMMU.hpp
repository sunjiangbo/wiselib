...
class BDMMU {
	...
	/*//typedef typename BlockMemory::size_t size_t; // TODO: funktioniert nicht...
	typedef uint32_t size_t;*/
	typedef typename BlockMemory::block_data_t block_data_t;
	typedef typename BlockMemory::self_pointer_t BlockMem;
	...
	BlockMem sd;
	Os::Debug::self_pointer_t debug_; //TODO: Get rid of Os:: ?
	int BLOCKS;
	...
	BDMMU(BlockMem sd, Os::Debug::self_pointer_t debug_, bool restore=true, bool persistent=true, bool *restoreSuccess = 0) : 		
	...
	bool erase(int start_vblock, int vblocks) {
		if(start_vblock >= 0 && start_vblock + vblocks < TOTAL_VBLOCKS) {
			...
			#ifdef BDMMU_DEBUG
				debug_->debug("VIRTUAL BLOCKS: sd->erase(%d, %d) | REAL BLOCKS: sd->erase(%d, %d)\n", start_vblock, vblocks, vr(start_vblock), vblocks * BLOCK_VIRTUALIZATION);
			#endif
	...
	bool read(block_data_t *buffer, int start_vblock, int vblocks) { 
		...
		if(start_vblock >= 0 && start_vblock + vblocks < TOTAL_VBLOCKS) {
			...
			sd->read(buffer, vr(start_vblock), vblocks * BLOCK_VIRTUALIZATION);
			
			#ifdef BDMMU_DEBUG
				debug_->debug("VIRTUAL BLOCKS: sd->read(buffer %d, %d) | REAL BLOCKS: sd->read(buffer %d, %d)\n", start_vblock, vblocks, vr(start_vblock), vblocks * BLOCK_VIRTUALIZATION);
			#endif
	...
	bool write(block_data_t *buffer, int start_vblock, int vblocks) {
		...
		if(start_vblock >= 0 && start_vblock + vblocks < TOTAL_VBLOCKS) { 
			...
			#ifdef BDMMU_DEBUG
				debug_->debug("VIRTUAL BLOCKS: sd->write(buffer %d, %d) | REAL BLOCKS: sd->write(buffer %d, %d)\n", start_vblock, vblocks, vr(start_vblock), vblocks * BLOCK_VIRTUALIZATION);
			#endif
